...
include('registros/medicamentos/estoque.php');
...
include('excluir/funcionarios/excluir_funcionarios.php');
?>
...
<?php
  ...
  while ($row = mysqli_fetch_assoc($return)) {
    ...
    <?php
     ...
     echo "Tratamento: <br>";
     $sql_estoque = "select quantidade, remedio_id_remedio,nome from estoque join remedio on id_remedio = remedio_id_remedio where quantidade = 0";
     $f = mysqli_query($conexao,$sql_estoque);

     echo "Remédios em falta:<br>";
     ...
     echo "<form action='alta.php' method='post'><button class='btn-sm btn-primary' type='submit' name='id_atendimento' value=".$row['id_atendimento'].">Dar Alta</button></form> <br>";
...
<?php
if ($_SESSION['id_cargo'] == 6 || $_SESSION['id_cargo'] == 1 || $_SESSION['id_cargo'] == 5 || $_SESSION['id_cargo'] == 8){
echo  "<h1>Adicionar:</h1>";
...
echo '<button type="button" class="btn-sm btn-primary" data-toggle="modal" data-target="#estoque">  Estoque</button>     ';
}
...
//echo "teste";
  ...
  <iframe src="listar_dados.php" name="listar-dados-bonitinho" scrolling="NO" width="100%" height="50%" allowtransparency="true" application="true"> </iframe>
